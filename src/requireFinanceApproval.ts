...
import type { TaskRequest, FinanceApprovalResult, FinanceAgentResponse } from './types.js';
...
const ALERT_SEVERITY_MAP: Record<string, 'info' | 'warning' | 'critical'> = {
...
const defaultLogger = {
  info: (msg: string, m?: Record<string, unknown>) => console.info(msg, m ?? ''),
  warn: (msg: string, m?: Record<string, unknown>) => console.warn(msg, m ?? ''),
  error: (msg: string, err?: unknown, m?: Record<string, unknown>) =>
    console.error(msg, err, m ?? ''),
  debug: (msg: string, m?: Record<string, unknown>) => console.debug(msg, m ?? ''),
};

/** Load bundled finance-agent.md governance file */
function loadBundledGovernance(): string {
 ...
 */
export async function requireFinanceApproval(
  request: TaskRequest,
  config: FinanceAgentConfig
): Promise<FinanceApprovalResult> {
  ...
  try {
    ...
    const globalBudget = (financialContext.budgets || []).find(
      (b: { scope: string; period: string }) => b.scope === 'global' && b.period === 'monthly'
    );
    const agentBudget = (financialContext.budgets || []).find(
      (b: { scope: string; period: string }) => b.scope === request.agent && b.period === 'monthly'
    );
    ...
    const currentMonthSpendEvents = (financialContext.spend_events || []).filter(
      (e: { created_at: string }) => new Date(e.created_at) >= currentMonthStart
    );
    const currentMonthSpend = currentMonthSpendEvents.reduce(
      (sum: number, e: { amount_usd: unknown }) => sum + Number(e.amount_usd || 0),
      0
    );
    const agentCurrentMonthSpend = currentMonthSpendEvents
      .filter((e: { agent: string }) => e.agent === request.agent)
      .reduce((sum: number, e: { amount_usd: unknown }) => sum + Number(e.amount_usd || 0), 0);

    const availableGlobalBudget = Math.max(0, monthlyLimit - currentMonthSpend);
    const availableAgentBudget =
      agentLimit > 0 ? Math.max(0, agentLimit - agentCurrentMonthSpend) : null;
    ...
    const resp = response as FinanceAgentResponse;
    ...
    if (resp.alerts_triggered && resp.alerts_triggered.length > 0) {
      await config.dataAdapter.insertAlerts(
        resp.alerts_triggered.map((alertType) => ({
          type: alertType,
