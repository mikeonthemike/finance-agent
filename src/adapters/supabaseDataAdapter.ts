 ...
 */
export function createSupabaseDataAdapter(supabase: SupabaseClient): DataAdapter {
  return {
    async getFinancialContext(): Promise<FinancialContext> {
      ...
      const spendEvents = (spendRes.data || []) as SpendEvent[];
      const revenueEvents = (revenueRes.data || []) as RevenueEvent[];
      ...
      const infraSpendFromEvents = infraSpendEvents.reduce(
        (s, e) => s + Number(e.amount_usd || 0),
        0
      );
      ...
      const infrastructureCost30d =
        infraSpendFromEvents > 0 ? infraSpendFromEvents : infraBudgetLimit;
    ...
    async submitRequest(request): Promise<string> {
      ...
      if (error) throw new Error(`Failed to submit task request: ${error.message}`);
      if (!data?.id) throw new Error('Failed to submit task request: no ID returned');
      return data.id;
    },
    ...
    async checkApproval(requestId) {
      ...
      if (error) throw new Error(`Failed to check approval: ${error.message}`);
      if (!data) throw new Error(`Request not found: ${requestId}`);

      return {
        approved: data.decision === 'approved',
        tier: data.approved_model_tier as ModelTier | undefined,
        reason: data.decision_reason ?? undefined,
      };
    },

    async recordResult(result): Promise<string | null> {
      const { data, error } = await supabase
      ...
      if (error) throw new Error(`Failed to record task result: ${error.message}`);
    ...
    async getRequest(requestId) {
      ...
      if (error) throw new Error(`Failed to get request: ${error.message}`);
      if (!data) throw new Error(`Request not found: ${requestId}`);
